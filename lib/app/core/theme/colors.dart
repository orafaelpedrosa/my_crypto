import 'package:flutter/material.dart';

class AppColors {
  static Color get widgetBackgroundColor => const Color(0xff1B0F3D);
  static Color get primaryColor => const Color(0xff190E3C);
  static Color get primaryBackgroundColor => const Color(0xff1B0F3D);
  static Color get secondBackgroundColor => const Color(0xff352E5A);
  static Color get primaryIconColor => const Color(0xff2666CF);
  static Color get primaryTextColor => Colors.white;
}
